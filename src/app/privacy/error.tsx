...
export default function PrivacyError({
  ...
  useEffect(() => {
    void fetch("/api/client-error", {
      ...
      body: JSON.stringify({ message: error.message, digest: error.digest, scope: "error-boundary.privacy" }),
    }).catch(() => {});
  }, [error]);
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center gap-4 p-8 text-center">
      ...
      <p className="text-base text-muted-foreground max-w-sm">
        No pudimos cargar la página de privacidad. Reintentá o volvé al inicio.
      </p>
      <button
        type="button"
        onClick={reset}
