// DELETE /api/integrations/logistica/disconnect — owner-only.
//
// Removes the CourierCredential row for the given provider.
...
import { VALID_PROVIDERS, type CourierProvider } from "../connect/_lib/credential-validators";
import {
  getServerActionMeta,
...
const MUTATION_ACTIONS = {
  DELETE: "courier_credential.disconnect",
...
export async function DELETE(req: NextRequest) {
  ...
  const typedProvider = provider as CourierProvider;

  // ── 4. Delete — idempotent (no row = still ok) ────────────────────────────
  try {
    await prisma.courierCredential.deleteMany({
      where: { businessId: ctx.businessId, provider: typedProvider },
    });
  } catch (err) {
    logRouteError("integrations/logistica/disconnect", err, { stage: "db_delete" });
  ...
  await recordCriticalWriteEvent({
    ...
    summary:         `Credencial de courier eliminada: ${typedProvider}`,
