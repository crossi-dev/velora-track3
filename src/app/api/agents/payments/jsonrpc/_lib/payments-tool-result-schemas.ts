...
// ---------------------------------------------------------------------------
// confirm_promesa_payment — output schemas
// ---------------------------------------------------------------------------

// Success path — promesa newly registered
export const ConfirmPromesaSuccessSchema = z.object({
...
// Replayed path — PaymentIntent was already confirmed (idempotency)
export const ConfirmPromesaReplayedSchema = z.object({
  success: z.literal(true),
  replayed: z.literal(true),
  paymentIntentId: z.string(),
  message: z.string(),
...
// Error path — always includes currency:"ARS" for LLM consistency
export const ConfirmPromesaErrorSchema = z.object({
  error: z.string(),
  message: z.string().optional(),
  currency: z.literal("ARS"),
});

// Union: success | replayed | error
export const ConfirmPromesaResultSchema = z.union([
  ConfirmPromesaSuccessSchema,
  ConfirmPromesaReplayedSchema,
  ConfirmPromesaErrorSchema,
]);

export type ConfirmPromesaResult = z.infer<typeof ConfirmPromesaResultSchema>;
