...
export function ProductDetailSheet({
  ...
  async function handleSave() {
    if (isEmployee || isSaving || productSavingRef.current) return;
    ...
    >
      <div className="flex flex-col gap-2 mb-4">
        {!isEmployee ? (
          ...
          <>
            <p className="text-body-strong" style={{ fontFamily: "var(--font-dm-sans)", fontWeight: 700, margin: 0 }}>
              {product.name}
            </p>
            <p className="text-caption" style={{ fontFamily: "var(--font-dm-sans)", color: "var(--tone-muted)" }}>
              {t("Stock:", "Stock:")} {product.stock} · ${product.price}
            </p>
            <p className="text-caption" style={{ fontFamily: "var(--font-dm-sans)", color: "var(--tone-muted)" }}>
              {t("Read only — ask the owner to make changes", "Solo lectura — pedile al dueño que haga el cambio")}
            </p>
          </>
        )}
